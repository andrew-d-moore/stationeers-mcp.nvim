...
function M.info(msg)
	vim.notify("[stationeers-mcp] " .. msg, vim.log.levels.INFO)
end
function M.warn(msg)
	vim.notify("[stationeers-mcp] " .. msg, vim.log.levels.WARN)
end
function M.error(msg)
	vim.notify("[stationeers-mcp] " .. msg, vim.log.levels.ERROR)
end
...
--- Indent a Lua table as pretty JSON without relying on jq or shell tricks.
local function pretty_json(val)
	if type(val) == "string" then
		-- Already a JSON string — decode first so we can re-encode with indent
	...
	-- Use jq via a temp file (works on Windows unlike herestrings)
	...
	local out = vim.fn.system("jq . " .. vim.fn.shellescape(tmp))
	vim.fn.delete(tmp)
	if vim.v.shell_error == 0 and out ~= "" then
		return out
	end
	-- jq not available — indent manually in Lua
	local indent = 0
	local result = {}
	local i = 1
	while i <= #encoded do
...
function M.show_result(title, content, filetype)
	...
	local lines
	if type(content) == "table" or (type(content) == "string" and filetype == "json") then
		lines = vim.split(pretty_json(content), "\n")
	else
